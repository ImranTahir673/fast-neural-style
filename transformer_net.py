...
# Standard Architecture (32, 64, 128 channels) - for new custom models
class TransformerNet(torch.nn.Module):
    def __init__(self):
        super(TransformerNet, self).__init__()
        # Initial convolution layers (standard configuration)
        ...
        # Residual layers (standard configuration)
        ...
        # Upsampling Layers (standard configuration)
...
# NC8 Architecture (8, 16, 32 channels) - for old pretrained models
class TransformerNetNC8(torch.nn.Module):
    def __init__(self):
        super(TransformerNetNC8, self).__init__()
        # Initial convolution layers (nc8 configuration)
        self.conv1 = ConvBlock(3, 8, kernel_size=9, stride=1)
        self.in1 = torch.nn.InstanceNorm2d(8, affine=True)
        self.conv2 = ConvBlock(8, 16, kernel_size=3, stride=2)
        self.in2 = torch.nn.InstanceNorm2d(16, affine=True)
        self.conv3 = ConvBlock(16, 32, kernel_size=3, stride=2)
        self.in3 = torch.nn.InstanceNorm2d(32, affine=True)
        
        # Residual layers (nc8 configuration)
        ...
        # Upsampling Layers (nc8 configuration)
        self.deconv1 = UpsampleConvBlock(32, 16, kernel_size=3, stride=1, upsample=2)
        self.in4 = torch.nn.InstanceNorm2d(16, affine=True)
        self.deconv2 = UpsampleConvBlock(16, 8, kernel_size=3, stride=1, upsample=2)
        self.in5 = torch.nn.InstanceNorm2d(8, affine=True)
        self.deconv3 = ConvBlock(8, 3, kernel_size=9, stride=1)
